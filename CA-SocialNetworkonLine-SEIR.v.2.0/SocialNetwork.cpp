...
SocialNetwork::SocialNetwork(): mN_Nodes(0), mN_Spreaders(0),
		mTimeStep(0), mVerbose(true), mK_Value(0.0), mVetProb(NULL),
		mEpsilon(0.0), mAveAlpha(0.0), mN_Alpha(0.0), mAveGamma(0.0), mN_Gamma(0.0),
	    mW_Alpha(0.0), mW_Gamma(0.0){
...
void SocialNetwork::loadConfig(const std::string& filename){
    ...
    mW_Alpha       = root["global-params"].get("alpha-weight", "UTF-8" ).asDouble();
    mW_Gamma       = root["global-params"].get("gamma-weight", "UTF-8" ).asDouble();
}
...
void SocialNetwork::initialCondition(void){
    ...
    graphFile.open(mFileGraph, std::fstream::in | std::fstream::binary);
    assert(graphFile.is_open());
    graphFile.read(reinterpret_cast<char *>(&mN_Nodes), sizeof(uint64_t));

    mGraph = make_shared<boost::adjacency_matrix<boost::directedS>> (mN_Nodes);
    boost::adjacency_matrix<boost::directedS> *pGraph = mGraph.get();
    uint64_t source = 0, target = 0;
    while (!graphFile.eof()){
        graphFile.read(reinterpret_cast<char *>(&source), sizeof(uint64_t));
        graphFile.read(reinterpret_cast<char *>(&target), sizeof(uint64_t));
...
double SocialNetwork::buildAlpha(uint64_t node, uint64_t iTime){
        ...
        if (k > 0.0)
            alpha = (1.0 - exp(-s/k)) -  (1.0 - exp(-r/k)); //alpha = (s/k) + (1.0 - exp(-c/k));
        else{
...
void SocialNetwork::print(void){
	...
	cout << " - Config:" << endl;
	cout << "\tNodes(" << mN_Nodes << ") | Timesteps(" << mTimeStep << ")" << endl;
	...
	cout << "\t - File graph info:         [" << mFileGraph << "]" << endl;
}
