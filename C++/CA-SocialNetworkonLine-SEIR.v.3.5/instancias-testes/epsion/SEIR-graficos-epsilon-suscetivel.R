...
print("SEIR MODEL - Análise do Epsilon no tempo - curva do suscetivel")
limits_x <- 30
...
g <- ggplot() +
  geom_point(data = df_10, aes(x = time, y = susceptible_percent,  color="df_10"), size=2.5)+
  geom_line(data = df_10, aes(x = time, y = susceptible_percent,  color="df_10"))+
  
  geom_point(data = df_25, aes(x = time, y = susceptible_percent,  color="df_25"), size=2.5) +
  geom_line(data = df_25, aes(x = time, y = susceptible_percent,  color="df_25"))+
  
  geom_point(data = df_50, aes(x = time, y = susceptible_percent,  color="df_50"), size=2.5) +
  geom_line(data = df_50, aes(x = time, y = susceptible_percent,  color="df_50"))+

  geom_point(data = df_75, aes(x = time, y = susceptible_percent,  color="df_75"), size=2.5) +
  geom_line(data = df_75, aes(x = time, y = susceptible_percent,  color="df_75"))+

  geom_point(data = df_90, aes(x = time, y = susceptible_percent,  color="df_90"), size=2.5) +
  geom_line(data = df_90, aes(x = time, y = susceptible_percent,  color="df_90"))+
  ...
  scale_color_manual(values = c (
    ...
    labels = c(
      df_10 = TeX("$\\lambda = 0.1$"),
      df_25 = TeX("$\\lambda = 0.25$"),
      df_50 = TeX("$\\lambda = 0.5$"),
      df_75 = TeX("$\\lambda = 0.75$"),
      df_90 = TeX("$\\lambda = 0.9$")
    )) +  # Custom colors
...
file_name_pdf <- "curva-epsilon-suscetivel.pdf"
