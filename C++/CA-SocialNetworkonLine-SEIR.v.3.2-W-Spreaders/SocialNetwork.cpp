...
void SocialNetwork::initMem(void){
	...
	mAlpha_Rumor_Ave =  mAlpha_Rumor_DP = mGamma_Rumor_Ave = 0.0;
	mGamma_Rumor_DP = mWSpreader = 0.0;
	mP_Spreaders =  mP_Infected = 0.0;
    mN_Spreaders  = mN_Infected = 0;
...
void SocialNetwork::loadConfig(const std::string& filename){
    ...
    mP_Spreaders = root["global-params"].get("spreaders", "UTF-8" ).asDouble();
    ...
    mWSpreader = root["global-params"].get("spreader-weight", "UTF-8" ).asDouble();
...
void SocialNetwork::createSpreader(uint64_t t){
	...
	for (uint64_t i = 0; i < mN_Nodes; i++){
	    if (mAgents[i].getSpreader()){
	        k_in += mAgents[i].getInDegree();
	        k_out += mAgents[i].getOutDegree();
	        k_samples++;
	    }//if (mAgents[i].getSpreader()){
    }//for (uint64_t i = 0; i < mN_Nodes; i++){
    k_in /= k_samples;
...
void SocialNetwork::initialCondition(void){
    ...
    start_time = std::chrono::high_resolution_clock::now();
    double value = floor(mP_Spreaders * static_cast<double> (mN_Nodes));
    mN_Spreaders = static_cast<uint64_t> (value) + 1;
    addSpreaders(mN_Spreaders);
    stop_time = std::chrono::high_resolution_clock::now();
    total_time = stop_time - start_time;
    std::cout << "\t - Add spreaders elapsed time:           " << setw(8) << fixed << setprecision(4) << total_time.count() << std::endl;
    std::cout.flush();
    ...
    for  (uint64_t node = 0; node < mN_Infected; node++){
        ...
        do{
            ...
            if ((!mAgents[n].getSpreader()) && (mAgents[n].getState() != STATES::INFECTED)){
    ...
    stop_time = std::chrono::high_resolution_clock::now();
	total_time = stop_time - start_time;
    std::cout << "\t - First agent infected elapsed time:    " << setw(8) << fixed << setprecision(4) << total_time.count() << std::endl;
	std::cout.flush();
...
void SocialNetwork::update(uint64_t t){
        ...
        //createSpreader(t);std::cout << "createSpreader 1" << std::endl; std::cout.flush();
		for (uint64_t i = 0; i < mN_Nodes; i++){
			mAgents[i].update();
		}
...
void SocialNetwork::print(void){
	...
	cout << "\tSpreaders percent : (" << setw(8) << fixed << mP_Spreaders << ") / Weight Spreaders(" << setw(8) << fixed << setprecision(4) << mWSpreader  << ")" << endl;
