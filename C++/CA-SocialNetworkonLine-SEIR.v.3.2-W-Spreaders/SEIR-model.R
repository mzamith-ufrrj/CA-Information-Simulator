...
print("SEIR MODEL")
file_name_dateset  <- "statistic.BA-CELL-1000000-P-1-C-6__Ep0_1__AAV-10_ASD-2_5__GAV-10-GSD-2_5.csv"
file_name_pdf <- "SEIR.BA-CELL-1000000-P-1-C-6__Ep0_1__AAV-10_ASD-2_5__GAV-10-GSD-2_5.pdf"
...
g <- ggplot() +
  geom_line(data = SN_data, aes(x = time, y = susceptible,  color="susceptible"))+
  geom_point(data = SN_data, aes(x = time, y = susceptible,  color="susceptible"), size=2.5) +
  
  geom_line(data = SN_data, aes(x = time, y = exposed,  color="exposed"))+
  geom_point(data = SN_data, aes(x = time, y = exposed,  color="exposed"), size=2.5) +
  
  geom_line(data = SN_data, aes(x = time, y = infected,  color="infected"))+
  geom_point(data = SN_data, aes(x = time, y = infected,  color="infected"), size=2.5) +
  
  geom_line(data = SN_data, aes(x = time, y = recovered,  color="recovered"))+
  geom_point(data = SN_data, aes(x = time, y = recovered,  color="recovered"), size=2.5) +
  ...
  labs(title = TeX("$\\epsilon$ = 0.1 / N($\\mu_{\\alpha}$ = 10, $\\sigma_{\\alpha}$ = 2.5) / N($\\mu_{\\gamma}$ = 10, $\\sigma_{\\gamma}$ = 2.5)"), color = "States:", x = "Time", y = "Cells (users)") +
